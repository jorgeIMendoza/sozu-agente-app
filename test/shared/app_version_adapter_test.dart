...
/// Este archivo existe por un fallo concreto, ocurrido en sozu-cliente-app: la
/// 1.0.3 llamaba a la function del gate sin `Authorization`, el gateway
/// respondia 401 antes de ejecutarla y el version gate quedaba MUDO - ni aviso
/// ni forzado - sin ninguna senal, porque el provider degrada a null ante
/// cualquier error.
///
/// Lo que se fija aqui son los HEADERS de la llamada, no la logica del gate
/// (esa vive en version_gate_test.dart): la function conserva
/// `verify_jwt = true`, asi que con solo `apikey` el gateway responde 401.
void main() {
  setUp(() {
    dotenv.testLoad(
      fileInput:
  ...
  test(
    ...
    () async {
      ...
      expect(
        ...
        'https://proyecto.supabase.co/functions/v1/agente-app-version',
      );
