...
String? _rutaAppDesdeUrl(String? url) {
  ...
  // Tolerar URLs absolutas del portal web (con prefijo del admin).
  const prefijo = '/admin/agent';
  ...
  // Detalle de proyecto: el portal web lo publica como /proyecto/:id y como
  // /inventario/proyecto/:id; el app solo tiene la segunda.
  final proyecto = RegExp(r'^(?:/inventario)?/proyecto/([^/]+)').firstMatch(u);
  if (proyecto != null) return '/inventario/proyecto/${proyecto.group(1)}';

  // Detalle de prospecto: misma ruta en el app.
  final prospecto = RegExp(r'^/prospectos/([^/]+)').firstMatch(u);
  if (prospecto != null) return '/prospectos/${prospecto.group(1)}';

  // Secundarias que se atienden con la ruta completa.
  const exactas = {
    '/inventario/unidades',
    '/perfil/cuenta',
    '/perfil/expediente',
  ...
  if (exactas.contains(u)) return u;

  // Tabs del portal, por su primer segmento.
  const directas = {
    '/inicio',
    '/inventario',
    '/pipeline',
