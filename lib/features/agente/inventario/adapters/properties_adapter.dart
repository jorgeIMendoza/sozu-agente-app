...
import 'package:sozu_agente_app/features/agente/inventario/ports/properties_port.dart';
...
class PropertiesAdapter implements PropertiesPort {
  ...
  Future<ClientePropiedades> properties() async =>
      ClientePropiedades.fromJson(await _invoke('agente-propiedades'));

  @override
  Future<PropiedadDetalle> property(int propertyId) async =>
      PropiedadDetalle.fromJson(
        await _invoke('agente-propiedad-detalle', body: {'id': propertyId}),
      );

  @override
  Future<ClientePagos> payments() async =>
      ClientePagos.fromJson(await _invoke('agente-pagos'));

  @override
  Future<EstadoCuenta> accountStatement(int accountId) async =>
      EstadoCuenta.fromJson(
        await _invoke('agente-estado-cuenta', body: {'id': accountId}),
      );

  @override
  Future<String?> accountStatementPdfUrl(int accountId) async =>
      _url(await _invoke('agente-estado-cuenta-pdf', body: {'id': accountId}));

  @override
  Future<DatosPago> paymentDetails(int agreementId) async => DatosPago.fromJson(
    await _invoke('agente-datos-pago', body: {'id': agreementId}),
  );

  @override
  Future<String?> paymentReceiptUrl(int paymentId) async =>
      _url(await _invoke('agente-recibo-pago', body: {'id': paymentId}));

  @override
  Future<List<BancoConvenio>> mortgageBanks() async {
    final res = await _invoke('agente-pago-final', body: {'action': 'bancos'});
