...
import 'package:sozu_agente_app/features/auth/services/biometric_service.dart';
import 'package:sozu_agente_app/features/auth/services/portal_access.dart';
import 'package:sozu_agente_app/features/auth/components/biometric_setup_sheet.dart';
import 'package:sozu_agente_app/core/version.dart';
import 'package:sozu_agente_app/features/auth/components/auth_alert.dart';
import 'package:sozu_agente_app/features/auth/components/auth_header.dart';
import 'package:sozu_agente_app/features/auth/layouts/auth_layout.dart';
...
/// rol de agente ([PortalAccess]: 3 Agente Inmobiliario o 9 Agente Interno). Si
/// no lo tiene, cierra sesión.
///
/// El destino sale del PERFIL, no de un gesto ni de la plataforma: con
/// `canManageAgentApp` va al selector de agentes, si no al portal. Mismo flujo
/// en web y en móvil.
...
class _LoginFormState extends ConsumerState<LoginForm> {
  ...
  /// enrolamiento viejo de una cuenta sin acceso al portal se apaga al entrar
  /// (`disableIfOwnedBy`).
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // Acceso administrador: requiere el permiso del rol (canManageAgentApp)
      ...
      // de agentes.
      final isAdminAccess =
          _isAdminMode && (profile?.canManageAgentApp ?? false);
      if (!PortalAccess.allows(profile) && !isAdminAccess) {
        // Ni agente (rol 3 o 9) ni administrador de
      ...
      // Un solo punto de salida para el resto: la oferta va igual para agente y
      ...
      await _offerBiometricsThenGo(
        auth,
        isAdminAccess ? '/seleccionar-agente' : '/inicio',
      );
    } catch (_) {
      await auth.signOut();
      auth.authFlowInProgress = false;
  ...
  /// agente y el de administrador: sin esto el admin nunca recibía la oferta,
  ...
  /// Login con huella/rostro, SOLO para agentes. El administrador entra siempre
  /// con correo y contraseña: su sesión puede impersonar a cualquier agente, y
  ...
  Future<void> _signInWithBiometrics({bool silentOnFailure = false}) async {
    ...
    try {
      ...
      if (!PortalAccess.allows(profile)) {
        // Enrolamiento viejo de una cuenta sin acceso: se apaga aquí. Dejarlo
        ...
        setState(() {
          ...
          _formError =
              'El acceso con huella es solo para agentes. Entra con tu correo '
              'y contraseña.';
        });
        return;
      }
      ...
      child: Form(
        ...
        child: AuthFormBody(
          children: [
            ...
            const AuthSubtitle('Entra a tu portal para vender y dar seguimiento.'),
