...
class AuthAdapter implements AuthPort {
  ...
  /// `roles.apps` es jsonb `{"administrar": ["clientes", "agentes", ...]}`.
  /// Se lee defensivamente: si el RPC aún no devuelve la columna o el rol no
  /// tiene el jsonb, no hay acceso administrador y el agente entra normal.
  static bool _administraAppAgentes(Object? apps) {
    if (apps is! Map) return false;
    final lista = apps['administrar'];
    return lista is List && lista.contains('agentes');
  }
  ...
  Future<UserProfile?> profile() async {
    try {
      ...
      return UserProfile(
        ...
        canManageAgentApp: _administraAppAgentes(row['apps']),
