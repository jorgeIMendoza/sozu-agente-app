...
actores (agente inmobiliario, agente interno y administrador de la app).
...
- La biometría guarda el refresh token en Keystore atado al `userId`
  ...
  cuenta sin acceso al portal al entrar).
- **Quién entra sale del PERFIL, no de la plataforma ni de un gesto.** El
  ...
  portal (`PortalAccess.allows`: rol 3 Agente Inmobiliario o 9 Agente Interno) →
  `debe_cambiar_password`. `canManageAgentApp` manda al selector de agentes.
...
- El modo administrador tiene DOS activaciones: long-press de 1.5 s sobre el
  ...
  Encenderlo NO concede nada: el acceso lo decide `canManageAgentApp` del
