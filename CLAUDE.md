# Proyecto: SOZU - Portal del Agente (app Flutter multiplataforma)

Port 1:1 del **Portal de Agentes del panel admin** (`sozu-admin`, rutas
`/admin/agent/*`): esa es la fuente funcional — lo que hace la web es lo que debe
hacer el app, pantalla por pantalla. La infraestructura (design system, CI, auth,
version gate, push) es hermana de `../sozu-cliente-app`; un arreglo de infra que
aplique a los dos va a los dos.
...
- **El `X.Y.Z` NO se sube a mano y el estado vive en los TAGS de git**, no en
  ...
  la versión anterior y ahí se quedaba (en sozu-cliente-app quedó web 1.0.3
  contra tiendas 1.0.4 sobre el mismo commit). Si se vuelve a tocar este paso, la
  regla es que quien compile primero tagea.
- El aviso in-app lo enciende `app_agente_config.latest_version`, que escribe el
...
## Rama de trabajo
`dev` es la rama de integración y `main` la de release; las dos están protegidas.
El trabajo sale en ramas `feat/*` o `fix/*` y entra por PR a `dev`; a `main` solo
se llega con un PR desde `dev`. NO se reescribe el historial: los mensajes de
commit son la documentación de por qué cada decisión se tomó, y esto va a
producción.
...
## Quién entra al Portal de Agentes
UN solo camino, el ROL (`features/auth/services/portal_access.dart`):
**Agente Inmobiliario (`roles.id` 3, el aliado externo) y Agente Interno
(`roles.id` 9, el de SOZU)**. A diferencia del Portal del Cliente —donde además
entra cualquiera con una compra activa, porque el rol dice para qué se contrató a
la persona y no si compró— aquí la única razón para entrar es que se te contrató
para vender. No hay equivalente de `es_comprador`.

⚠️ **El gate está DUPLICADO y los dos lados se cambian juntos.** El de verdad es
`_shared/agente.ts` → `authAgente()` en `sozu-edge-functions`: si el frontend
deja pasar y el backend no, el usuario entra y recibe **403 en cada pantalla**.
`PortalAccess.allows` es el espejo de ese gate y
`test/features/auth/portal_access_test.dart` fija el contrato.

**Dependiente vs independiente** es otra cosa, y NO se decide en el app: un rol 3
con `entidades_relacionadas` tipo 19 y `id_persona_duena_lead` no nulo cuelga de
una inmobiliaria, así que el portal le esconde Comisiones y le pone fiscal y banco
en solo lectura ("La administra {inmobiliaria}"). Eso lo resuelve el servidor y
llega en `agente-sesion` → `restricciones`; la UI solo lo obedece.

El acceso administrador ("ver como agente") es OTRA COSA: va por
`canManageAgentApp` (`roles.apps.administrar` incluye `agentes`), no por aquí.

## Reglas de SEGURIDAD (innegociables)
- SOLO Supabase ANON KEY (pública) + JWT del usuario logueado.
- NUNCA service_role ni credenciales de BD en el código.
- CERO queries a tablas: todo dato va por Edge Functions `agente-*`
  (`agente-sesion`, `agente-inicio`, `agente-inventario`, `agente-prospectos`,
  `agente-pipeline`, `agente-comisiones`, `agente-perfil`, `agente-citas`,
  `agente-telemetria`, `agente-push-token`, `agente-app-version`) + 2 RPC
  SECURITY DEFINER (`get_current_user_profile`, `mark_password_changed`).
  El único llamador es `lib/shared/adapters/edge_function.dart`; las públicas
  pre-login (recuperar contraseña, version gate) van por
  `lib/shared/adapters/anon_function.dart`.
- La identidad del agente la deriva el backend del JWT: el app NUNCA manda
  `email`, `id_persona` ni `auth_user_id` en el body. Mandarlos es una vía para
  leer los datos de otro agente.
...
screens/select_agente_screen.dart · announcements_screen.dart
components/admin_header_bar.dart · agente_filters.dart · agente_row.dart
```
...
```dart
import 'package:sozu_agente_app/ui/ui.dart';   // ✅ el equivalente de @/ui en TS
import '../../../../ui/ui.dart';                 // ❌
```
...
✅ **`--no-fatal-infos` vuelve a respetar el código de salida.** Medido el
2026-08-12 sobre `main`: `flutter analyze --no-fatal-infos` devuelve **0** con
241 infos. La nota anterior decía lo contrario (comprobado el 2026-08-07, cuando
devolvía 1 aunque solo hubiera infos) y de ahí salía el pendiente de "arreglar
`check.sh` para que cuente errores en vez de mirar el exit code": ese trabajo ya
no hace falta. Si `check.sh` vuelve a marcar `✗ analyze encontro errores o
warnings`, hay algo fatal de verdad y se ve con
`flutter analyze 2>&1 | grep -E '^\s+(error|warning)'`.
...
- `lib/screens/` y `lib/providers/` YA NO EXISTEN: sus pantallas viven en
  `features/agente/*/screens/` y sus providers en la feature que los usa.
...
El selector vive en Perfil (`features/agente/profile/components/theme_selector.dart`)
